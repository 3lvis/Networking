...
4. **Request/response middleware.** A composable pipeline around every request — table stakes for a serious client. A short PR series; the seam shape was settled with a prior-art scan (chose the swift-openapi `ClientMiddleware` async-`next` onion over Alamofire's two-protocol adapt/retry split — one concept covers adapt + retry + auth).
    ...
    - ~~**4c — Retry policy.**~~ ✅ `RetryInterceptor`: exponential backoff + full jitter, `maxAttempts` cap, `Retry-After` support (seconds or HTTP-date), driven by the `isRetryable` classifier from #2 (transient transport + `retryableStatusCodes`). Retryability predicates extracted onto `NetworkingError` as the single source of truth. **Downloads now route through the chain** (`requestData` → `perform`), so retry/auth apply to them too.
