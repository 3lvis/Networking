...
7. ~~**Full tooling upgrade — Swift 6 language mode, keep iOS 18.**~~ ✅ Done, and more contained than feared (one PR, not a series). Flipped `swiftLanguageModes` to `[.v6]`. The strict-concurrency fallout was small and centred on `Sendable`:
   ...
   - Pinned `swift-tools-version: 6.2` — the latest CI can parse. The `macos-15` runner ships Xcode up to **26.3** (verified against the runner-images manifest; not 26.5, which is a local toolchain), but Xcode 26.3 still bundles **Swift 6.2.x** — Swift 6.3 (what a `6.3` manifest needs) first ships in Xcode 26.4, which isn't on the image yet. CI currently pins Xcode 26.2 (Swift 6.2.3, per its `Versions` log); bumping the pin to 26.3 is possible but wouldn't change the Swift version, so `6.2` stays the ceiling.
