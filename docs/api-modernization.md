...
- [ ] **Migrate the downloads to `Result`, then delete the last result/response hierarchy.** `downloadImage`/`downloadData` are the only remaining users of the bespoke result model — and they're oddly `async throws -> ImageResult`/`DataResult` (throwing *and* returning a result enum). Migrate them to `Result<Image, NetworkingError>` / `Result<Data, NetworkingError>` (Swift's stdlib `Result`, matching every other verb), then delete the `NetworkingResult` protocol, `ImageResult`, `DataResult`, and the `Response`/`SuccessImageResponse`/`SuccessDataResponse`/`FailureResponse` classes. Public API change → its own PR. Afterward the library has one result type everywhere.
- [ ] **Drop the internal `JSON` enum.** `JSON` (`JSON.swift`) is an internal parsing helper still used by file-based fakes (`FileManager.json`) and the cache path in `Networking+Private.swift`. Replace those uses with `JSONSerialization`/`Codable` directly and delete the enum (+ `JSONTests`/`JSONIntegrationTests` that test it). Internal-only, no public surface — lower priority than the download migration, do it after.
