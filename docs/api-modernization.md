...
7. ~~**Full tooling upgrade — Swift 6 language mode, keep iOS 18.**~~ ✅ Done, and more contained than feared (one PR, not a series). Flipped `swiftLanguageModes` to `[.v6]`. The strict-concurrency fallout was small and centred on `Sendable`:
   ...
   - `Networking` → a real **`actor`** (see #8 — initially shipped as `@unchecked Sendable`, then made a true actor after review flagged the conformance as unsound).
   ...
   - Pinned `swift-tools-version: 6.2` — the latest CI can parse. The `macos-15` runner ships Xcode up to **26.3** (verified against the runner-images manifest; not 26.5, which is a local toolchain), but Xcode 26.3 still bundles **Swift 6.2.x** — Swift 6.3 (what a `6.3` manifest needs) first ships in Xcode 26.4, not on the image yet. CI's `DEVELOPER_DIR` pin bumped 26.2 → 26.3 (runner's newest; same Swift 6.2.x).

8. ~~**Rewrite `Networking` as an `actor` (replace the `@unchecked Sendable`).**~~ ✅ Done (in the same PR as #7, after review flagged the `@unchecked` conformance as unsound — `fakeRequests`/config could race with in-flight requests). `open class` → `public actor`, so isolation is **compiler-verified**. Breaking API changes:
   - **No subclassing** (actors can't be subclassed; was `open`).
   - **Config setters are async + method-based:** `await networking.setAuthorizationHeader(...)`, and new `setHeaderFields(_:)` / `setUnauthorizedRequestCallback(_:)` / `setErrorLoggingEnabled(_:)` replace direct property assignment (actors forbid external mutation of isolated stored properties). `unauthorizedRequestCallback` is now `@Sendable`.
   - Thread-safe/immutable members kept `nonisolated` so they need no `await` and don't cross the actor boundary: `cache` (`nonisolated(unsafe)`, NSCache is thread-safe), `boundary`, `composedURL`/`destinationURL`/`objectFromCache`/`imageFromCache`/`dataFromCache`/`cacheOrPurge*`.
   - The `@unchecked Sendable` on `AnyCodable`/`ImageResponse` stays — those are immutable value types, low-risk.
   - Removed the dead `public var isSynchronous` — it was declared but never read anywhere (a no-op flag from the pre-async era), so rather than add a `setSynchronous(_:)` for a setting that does nothing, it's deleted.
