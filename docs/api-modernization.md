...
2. **Rename `NetworkingResponse` → `JSONResponse`.** Once the downloads have `ImageResponse`/`DataResponse`, the JSON envelope should be named symmetrically: `JSONResponse` (`{ statusCode, headers, body }`). Mechanical but broad — touches every verb test/call site that uses the envelope (dozens) — so it's its **own follow-up PR** right after #1 to keep both reviewable. Update the README envelope examples too.
