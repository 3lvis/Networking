...
func row(_ name: String, _ total: Double, ops: Int) {
    let nsPerOp = total / Double(ops) * 1e9
    let opsPerSec = Double(ops) / total
    print(String(format: "  %-34s %8.1f ns/op   %12.0f ops/sec   (%.3fs total)", (name as NSString).utf8String!, nsPerOp, opsPerSec, total))
}
