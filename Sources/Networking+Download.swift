...
public extension Networking {
    ...
    /**
     Downloads an image using the specified path.
     - parameter path: The path where the image is located.
     - parameter completion: A closure that returns the url (URL?) and the error (NSError?)
     */
    public func downloadFile(_ path: String, completion: @escaping (_ url: URL?, _ error: NSError?) -> Void) {
        self.request(.GET, path: path, cacheName: nil, parameterType: nil, parameters: nil, parts: nil, responseType: .data) { response, _, error in
            TestCheck.testBlock(self.disableTestingMode) {
                completion(response as? URL, error)
            }
        }
    }
    ...
    /**
     ...
     - parameter completion: A closure that gets called when the cancellation is completed.
     */
    public func cancelFileDownload(_ path: String, completion: ((Void) -> Void)? = nil) {
        let url = self.url(for: path)
        self.cancelRequest(.data, requestType: .GET, url: url, completion: completion)
    }
    ...
    /**
     ...
     - parameter image: An image that will be returned when there's a request to the registered path.
     */
    public func fakeFileDownload(_ path: String, url: URL?, statusCode: Int = 200) {
        self.fake(.GET, path: path, response: url, responseType: .data, statusCode: statusCode)
    }
}
