...
public extension Networking {
    ...
    @discardableResult
    func altGET(_ path: String, parameters: Any? = nil, cachingLevel: CachingLevel = .none, completion: @escaping (_ result: Result<JSONResponse, NSError>) -> Void) -> String {
        let parameterType: ParameterType = parameters != nil ? .formURLEncoded : .none

        return altHandleJSONRequest(.get, path: path, cacheName: nil, parameterType: parameterType, parameters: parameters, responseType: .json, cachingLevel: cachingLevel, completion: completion)
    }
