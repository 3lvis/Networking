...
extension Networking {
    ...
    public func get<Q: Encodable, T: Decodable>(
        _ path: String,
        query: Q,
        cachingLevel: CachingLevel = .none
    ) async
    ...
    public func post<B: Encodable>(_ path: String, body: B) async -> Result<Void, NetworkingError> {
        let result: Result<Data, NetworkingError> = await encodeAndHandle(
            .post,
            path: path,
            body: body
        )
    ...
    public func post<F: Encodable, T: Decodable>(_ path: String, form: F) async -> Result<T, NetworkingError> {
        return await formEncodeAndHandle(
            .post,
            path: path,
            form: form
        )
    ...
    public func post<F: Encodable>(_ path: String, form: F) async -> Result<Void, NetworkingError> {
        let result: Result<Data, NetworkingError> = await formEncodeAndHandle(
            .post,
            path: path,
            form: form
        )
...
extension Networking {
    ...
    func encodeAndHandle<B: Encodable, T: Decodable>(
...
extension Networking {
    func formEncodeAndHandle<T: Decodable>(
        _ requestType: RequestType,
        path: String,
        form: some Encodable
    ) async
